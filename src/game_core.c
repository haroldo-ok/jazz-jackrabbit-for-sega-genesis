...
extern u8 jj1_runtime_solid(u8 stage, s16 tx, s16 ty);
extern u8 jj1_runtime_rect_solid(u8 stage, s16 x, s16 y, s16 w, s16 h);
extern u8 jj1_runtime_point_solid(u8 stage, s16 x, s16 y);
extern u8 jj1_runtime_down_point_solid(u8 stage, s16 x, s16 y);
extern s8 jj1_runtime_jump_velocity(u8 stage);
extern s8 jj1_runtime_spring_velocity(u8 stage, s16 x, s16 y, s16 w, s16 h);
extern void jj1_runtime_place_player(JazzGame *game, u8 stage);
#else
...
static void build_stage(JazzGame *g, u8 stage)
{
    ...
    if (stage == 0) {
        ...
        add_platform(g, 69, 10, 7); add_platform(g, 82, 9, 5);
        for (i = 0; i < 18; i++) spawn_gem(g, i, 5 + (i * 5), (i & 1) ? 11 : 12);
        spawn_enemy(g, 0, 18, 16, 22); spawn_enemy(g, 1, 34, 31, 38);
        spawn_enemy(g, 2, 61, 58, 67); spawn_enemy(g, 3, 79, 76, 83);
    } else if (stage == 1) {
        add_platform(g, 8, 11, 6); add_platform(g, 19, 8, 5);
        add_platform(g, 31, 10, 7); add_platform(g, 46, 9, 5);
        add_platform(g, 58, 7, 6); add_platform(g, 73, 10, 8);
        for (i = 0; i < 20; i++) spawn_gem(g, i, 4 + (i * 4), (i % 3) ? 12 : 10);
        spawn_enemy(g, 0, 15, 12, 18); spawn_enemy(g, 1, 28, 25, 32);
        spawn_enemy(g, 2, 44, 40, 48); spawn_enemy(g, 3, 66, 62, 71);
        spawn_enemy(g, 4, 84, 80, 88);
    } else {
        add_platform(g, 11, 9, 5); add_platform(g, 25, 11, 6);
        add_platform(g, 40, 8, 7); add_platform(g, 56, 10, 5);
        add_platform(g, 68, 7, 7); add_platform(g, 80, 10, 7);
        for (i = 0; i < 22; i++) spawn_gem(g, i, 4 + (i * 4), (i & 1) ? 12 : 11);
        spawn_enemy(g, 0, 17, 14, 21); spawn_enemy(g, 1, 36, 32, 39);
        spawn_enemy(g, 2, 52, 49, 56); spawn_enemy(g, 3, 65, 61, 69);
        spawn_enemy(g, 4, 78, 74, 82); spawn_enemy(g, 5, 89, 85, 91);
    }
...
static void update_bullets(JazzGame *g)
{
    ...
    for (i = 0; i < JAZZ_MAX_BULLETS; i++) {
        ...
        for (e = 0; e < JAZZ_MAX_ENEMIES; e++) {
            if (g->enemies[e].active && rect_overlap(b->x, b->y, 4, 4, g->enemies[e].x, g->enemies[e].y, ENEMY_W, ENEMY_H)) {
                b->active = 0;
...
static void update_enemies(JazzGame *g)
{
    ...
    for (i = 0; i < JAZZ_MAX_ENEMIES; i++) {
        ...
        e->x += e->direction;
        if ((e->x >> 4) <= e->patrolLeft) e->direction = 1;
        if ((e->x >> 4) >= e->patrolRight) e->direction = -1;
...
void jazz_step(JazzGame *g, u16 input)
{
    ...
    if (pressed & JAZZ_INPUT_START) { g->paused = !g->paused; return; }
    ...
    target = 0;
    if (input & JAZZ_INPUT_LEFT) target = -3;
    if (input & JAZZ_INPUT_RIGHT) target = 3;
    if (target) {
        g->player.vx = target;
        g->player.facing = (target > 0);
    } else if (g->player.vx > 0) g->player.vx--;
    else if (g->player.vx < 0) g->player.vx++;

    if ((pressed & JAZZ_INPUT_JUMP) && g->player.onGround) {
#ifdef JAZZ_JJ1_RUNTIME
        g->player.vy = jj1_runtime_jump_velocity(g->stage);
#else
        g->player.vy = -9;
#endif
        g->player.onGround = 0;
    ...
    move_player_x(g, g->player.vx);
    if (g->player.vy < 6) g->player.vy++;
    move_player_y(g, g->player.vy);
#ifdef JAZZ_JJ1_RUNTIME
    {
        s8 spring = jj1_runtime_spring_velocity(g->stage, g->player.x, g->player.y, PLAYER_W, PLAYER_H);
        if (spring) {
            g->player.vy = spring;
