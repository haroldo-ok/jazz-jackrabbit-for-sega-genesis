...
# SGDK ships 68000-safe arithmetic helpers. Debian's m68k-linux-gnu libgcc
# targets later 68k CPUs and emits BSR.L, which faults on a stock Genesis.
LIBGCC := $(LIB)/libgcc.a
...
# GCC 14's store-merging pass can emit unaligned word/long stores for byte arrays.
# That is legal on some m68k variants but faults on the Genesis 68000.
DEFAULT_FLAGS= $(EXTRA_FLAGS) -DSGDK_GCC -DJAZZ_JJ1_RUNTIME -m68000 -mstrict-align -fno-store-merging -fno-tree-slp-vectorize -fdiagnostics-color=always -Wall -Wextra -Wno-shift-negative-value -Wno-main -Wno-unused-parameter -fno-builtin -ffunction-sections -fdata-sections -fms-extensions $(INCS) -B$(BIN)
...
$(OUT)/rom.out: $(OUT)/sega.o $(OUT)/cmd_ $(LIBMD)
	$(MKDIR) -p $(dir $@)
	$(CC) -m68000 -B$(BIN) -n -T $(GDK)/md.ld -nostdlib $(OUT)/sega.o @$(OUT)/cmd_ $(LIBMD) $(LIBGCC) -o $(OUT)/rom.out -Wl,--gc-sections -Wl,--build-id=none -flto -flto=auto -ffat-lto-objects
	$(RM) $(OUT)/cmd_
